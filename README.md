...
Явных захардкоженных API-ключей в рабочем дереве не должно быть, но перед публичным push важно убрать из git index приватные/тяжелые артефакты. `.gitignore` уже защищает от повторного добавления, однако файлы из первого хакатонного коммита остаются tracked, пока их явно не удалить из индекса.
...
Команды выше не удаляют локальные файлы, если использовать `--cached`: они только перестают попадать в публичный репозиторий. Если лицензия разрешает публиковать часть данных, это стоит явно описать в `data/README.md`.

## ✅ Что важно

- Runtime detector не вызывает внешние API.
- RAG и внешняя база знаний не используются.
- Fine-tuning GigaChat не требуется.
- Классификатор маленький, поэтому overhead после forward pass остается низким.
- Основная ценность проекта: аккуратное использование внутренних сигналов LLM вместо дорогой внешней верификации.
